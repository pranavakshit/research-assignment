...
def plot_model_performance():
    ...
    for i, metric in enumerate(metrics):
        ...
        rects = ax.bar(
            ...
            width,
            yerr=stds[i],
            label=f"Spam {metric}" if metric != 'Accuracy' else 'Accuracy',
            color=colors[i],
            capsize=3,
            edgecolor='black',
            linewidth=0.6,
        ...
        for rect, val in zip(rects, means[i]):
            ax.annotate(
                f"{val:.1f}%",
                xy=(rect.get_x() + rect.get_width() / 2, rect.get_height() + 1.0),
                xytext=(0, 0),
    ...
    ax.set_title('Figure 1: Classifier Performance Comparison (Mean ± Std, 3 Seeds)', fontsize=10.5, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(["Multinomial NB", "Logistic Reg.", "Linear SVM"], fontweight='bold', fontsize=9)
    ax.set_ylim(70, 106)
    ax.legend(loc='lower right', frameon=True, framealpha=0.9, fontsize=8.5, ncol=2)
    plt.tight_layout()
    ...
    plt.close()
    print(" -> Generated compact: fig1_model_performance.pdf / .png")
...
def plot_ablation_impact():
    ...
    models = ["Multinomial NB", "Logistic Reg.", "Linear SVM"]
    ...
    rects1 = ax.bar(x - width/2, f1_en_mean, width, yerr=f1_en_std, label='Preprocessing Enabled',
                    color='#3274a1', capsize=3, edgecolor='black', linewidth=0.6)
    rects2 = ax.bar(x + width/2, f1_dis_mean, width, yerr=f1_dis_std, label='Preprocessing Disabled (Raw)',
                    color='#e1812c', capsize=3, edgecolor='black', linewidth=0.6)

    for rect, val in zip(rects1, f1_en_mean):
        ax.annotate(f"{val:.1f}%", xy=(rect.get_x() + rect.get_width() / 2, rect.get_height() + 0.7),
                    xytext=(0, 0), textcoords="offset points", ha='center', va='bottom', fontsize=8)

    for rect, val in zip(rects2, f1_dis_mean):
        ax.annotate(f"{val:.1f}%", xy=(rect.get_x() + rect.get_width() / 2, rect.get_height() + 0.7),
                    xytext=(0, 0), textcoords="offset points", ha='center', va='bottom', fontsize=8)

    ax.set_ylabel('Spam F1-Score (%)', fontsize=9.5)
    ax.set_title('Figure 2: Preprocessing Ablation Impact on Spam F1 (Mean ± Std)', fontsize=10.5, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.close()
    print(" -> Generated compact: fig2_ablation_impact.pdf / .png")
...
def plot_confusion_matrices():
    ...
    fig, axes = plt.subplots(1, 3, figsize=(10.2, 2.7), dpi=300)
    ...
    for idx, (m_name, ax) in enumerate(zip(models, axes)):
        ...
        for i in range(2):
            for j in range(2):
                pct = cm_norm[i, j] * 100
                ax.text(j + 0.5, i + 0.75, f"({pct:.1f}%)", ha='center', va='center', color='darkblue' if cm[i,j] < 500 else 'white', fontsize=8)

        ax.set_title(f"{m_name}\n(Acc: {run_data['models'][m_name]['accuracy']*100:.1f}%)", fontsize=9.5, fontweight='bold')
        ax.set_ylabel('Ground Truth' if idx == 0 else '', fontsize=9)
        ax.set_xlabel('Predicted Label', fontsize=9)

    plt.suptitle('Figure 3: Test-Set Confusion Matrices (1,115 messages, Seed 42)', fontsize=11, y=1.03)
    plt.tight_layout()
    ...
    plt.close()
    print(" -> Generated compact: fig3_confusion_matrices.pdf / .png")
